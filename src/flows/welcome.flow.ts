...
export const welcomeFlow = addKeyword(EVENTS.WELCOME)
  .addAction(async (ctx, { flowDynamic, extensions }) => {
    ...
    try {
      ...
      console.warn('[welcome] Gemini falló, usando fallback hardcodeado')
    }

    await flowDynamic([{ body: greeting ?? 'Bienvenido al CEE de la FIIS-UNI. Responde *1*, *2* o *3*.', delay: rnd() }])
  })
