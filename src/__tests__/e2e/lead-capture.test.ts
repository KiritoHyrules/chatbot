...
describe('E2E — lead capture', () => {
  ...
  it('DNI con puntos se normaliza a 8 dígitos', () => {
  ...
  it('DNI con guiones se normaliza a 8 dígitos', () => {
    const input = '12-345-678'
    const normalized = input.replace(/[\s.\-]/g, '')
    expect(normalized).toBe('12345678')
    expect(/^\d{8}$/.test(normalized)).toBe(true)
  ...
  it('email con "arroba" y "punto" se detecta como inválido', () => {
    const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    expect(re.test('juan.perez(arroba)gmail(punto)com')).toBe(false)
    expect(re.test('juan(arroba)correo(punto)com')).toBe(false)
  })
...
describe('lead-capture — intent escape', () => {
  it('pregunta de costo durante captura se responde con KB', () => {
    const answer = findAnswer('costo del diplomado')
    expect(answer).toBeTruthy()
    expect(typeof answer).toBe('string')
  })

  it('pregunta de duración se responde con KB', () => {
    const answer = findAnswer('cuánto dura el programa', 'Diplomado en Ciencia de Datos')
    expect(answer).toBeTruthy()
    expect(answer).toContain('dura')
  })

  it('escapeCount trackea múltiples escapes', () => {
    let escapeCount = 0
    escapeCount++
