...
export const dashboard = {
  addMessage(phone: string, role: 'user' | 'assistant' | 'human', content: string, name?: string) {
    const ts = new Date().toISOString()
    run('INSERT INTO dashboard_messages (phone, name, role, content, ts) VALUES (?, ?, ?, ?, ?)', [phone, name ?? null, role, content, ts])
...
export const outbox = {
  ...
  getPending(limit = 10) {
    return all<{ id: number; event: string; payload: string; attempts: number }>(
      'SELECT * FROM outbox WHERE status = ? ORDER BY created_at ASC LIMIT ?', ['pending', limit])
  },
  ...
  markFailed(id: number) {
    run(`UPDATE outbox SET attempts = attempts + 1, status = CASE WHEN attempts + 1 >= 5 THEN 'failed' ELSE 'pending' END WHERE id = ?`, [id])
  },
}
