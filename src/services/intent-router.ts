...
export const intentRouter = {
  detect(message: string, hasActiveFlow: boolean = false): { intent: Intent; number?: number; confidence: 'ALTA' | 'MEDIA' | 'BAJA' } {
    ...
    if (/\b(ver|conocer|cat[aá]logo|listado|mostrar|ense[ñn]ar|tienes|tienen|hay|c[uú]ales|saber|quiero)\b/i.test(text) &&
        /\b(programas?|cursos?|diplomados?|talleres?|pee|cat[aá]logos?)\b/i.test(text)) {
    ...
    // Capa 6b: Verbos de petición + alusión a programas → programs (ALTA)
    if (/\b(hablame|cuentame|cuéntame|dime|explicame|explícame|muestrame|muéstrame|enséñame|ensename)\b/i.test(text) &&
        /\b(cursos?|programas?|diplomados?|talleres?)\b/i.test(text)) {
      return { intent: 'programs', confidence: 'ALTA' }
    }

    // Capa 7: Frases de exploración genérica
    if (/\b(cursos?|programas?|diplomados?|talleres?)\b/i.test(text)) {
